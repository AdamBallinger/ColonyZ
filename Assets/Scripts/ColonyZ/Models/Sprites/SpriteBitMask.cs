...
namespace ColonyZ.Models.Sprites
{
    ...
    public static class SpriteBitMask
    {
        ...
        public static int GetObjectWorldIndex(Tile _tile)
        {
            ...
            if (Connects(tile_NW) && Connects(tile_N) && Connects(tile_W)) bitmaskValue += 1;

            if (Connects(tile_N)) bitmaskValue += 2;

            if (Connects(tile_NE) && Connects(tile_N) && Connects(tile_E)) bitmaskValue += 4;

            if (Connects(tile_W)) bitmaskValue += 8;

            if (Connects(tile_E)) bitmaskValue += 16;

            if (Connects(tile_SW) && Connects(tile_S) && Connects(tile_W)) bitmaskValue += 32;

            if (Connects(tile_S)) bitmaskValue += 64;

            if (Connects(tile_SE) && Connects(tile_S) && Connects(tile_E)) bitmaskValue += 128;
        ...
        {
            if (_neighbour == null || !tile.HasObject || !_neighbour.HasObject) return false;
